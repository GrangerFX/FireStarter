...
#define FIRESTARTER_OPTIMIZE_GPU_PASSES          100
...
#define FIRESTARTER_OPTIMIZE_GPU_OPTIMIZE        4
...
#define FIRESTARTER_POPULATION      FIRESTARTER_OPTIMIZE_CPU_POPULATIO
...
#define FIRESTARTER_POPULATION      FIRESTARTER_OPTIMIZE_GPU_POPULATIO
...
public:
    ...
    inline const FireStarterSetting& SetSettings(unsigned int mode = FIRESTARTER_AUTO)
    {
        if (mode == FIRESTARTER_AUTO)
            mode = m_mode;
        else if (mode >= FIRESTARTER_MODES)
            mode = FIRESTARTER_SOLUTION;
        ::FireStarterSetting() = m_settings[mode];
    } // SetSettings
