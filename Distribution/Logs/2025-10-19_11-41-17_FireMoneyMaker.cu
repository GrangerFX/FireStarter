...
inline bool MoneyMakerEvaluate(const FireStarterData& data, const FireStarterCode& code, const MoneyMakerStock& stockData, const unsigned int stockStart, unsigned int& trades, float& result)
{
...
GPU_GLOBAL void MoneyMaker(float* results, FireStarterResult* population, FireStarterCode* codes, MoneyMakerStocks* stocks, const unsigned long long seed, const unsigned int passes, const unsigned int populationSize, const unsigned int stock)
{
    ...
    unsigned long long stockSeed = seed + SEED2(stock);     // Same seed for all members
    unsigned int stockStart = RANDOMMOD(stockSeed, MONEYMAKER_VARIATION + 1);
    ...
    for (unsigned int i = 0; i < 10; i++) {
        ...
        if (MoneyMakerEvaluate(data, code, stockData, stockStart, memberTrades, memberResult))
            break;
    }
    ...
    for (unsigned int pass = 0; pass < passes; pass++) {
        ...
        for (unsigned int i = 0; i < FIRESTARTER_EVOLVE_GPU_ITERATIONS; i++) {
            ...
            if (MoneyMakerEvaluate(data, code, stockData, stockStart, curTrades, curResult)) {
