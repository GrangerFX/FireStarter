...
// EVALUATE //
// Run date: 04/14/24 10:48:27 Pacific Daylight Time
// Run duration = 251.225416 seconds
// Run generation = 91
// Run evolution = 4
...
GPU_GLOBAL void Optimizer(const FireStarterSettings settings, FireStarterPopulation* newResults, const FireStarterPopulation* oldResults, const unsigned int v, const unsigned int dataSize, const unsigned long long optimizationSeed, const unsigned long long optimizationPass)
{
    ...
    if (!optimizationPass || (result < oldResult))
        ...
        if (bestCandidate == member)
            ...
            unsigned int oldIndex = oldResults->Index(member, v);
            const FireStarterData* oldData = oldResults->Data(bestCandidate, v);
            newResults->InitResult(member, v, MAX(oldIndex, 1) + 1, FIRESTARTER_REGISTERS, oldResult, oldData);
        }
    }
    if (!member && !v)
