...
inline bool MoneyOptimizeEvaluate(const FireStarterData& data, const MoneyMakerStock& stock, const unsigned int stockStart, float& result)
{
...
inline bool MoneyOptimizeEvaluateStocks(const FireStarterData& data, const MoneyMakerStocks* stocks, const unsigned int* stockStart, float& result)
{
    float stocksResult = 0.0f;
    for (unsigned int stock = 0; stock < MONEYMAKER_STOCKS; stock++) {
        float stockResult = FIRESTARTER_START_RESULT;
        if (!MoneyOptimizeEvaluate(data, stocks->Stock(stock), stockStart[stock], stockResult)) {
            stocksResult = FIRESTARTER_START_RESULT;
            break;
        }
        stocksResult += stockResult / MONEYMAKER_STOCKS;
    }
...
GPU_GLOBAL void MoneyOptimizer(FireStarterResult* newPopulation, const FireStarterResult* oldPopulation, MoneyMakerStocks* stocks, const unsigned int registers, const unsigned long long evolveSeed, const unsigned long long optimizeSeed, const unsigned long long optimizePass, unsigned int populationSize)
{
    ...
    unsigned int stockStart[MONEYMAKER_STOCKS];
    for (unsigned int stock = 0; stock < MONEYMAKER_STOCKS; stock++) {
        unsigned long long stockSeed = evolveSeed + SEED2(stock);     // Same seed for all members
        stockStart[stock] = RANDOMMOD(stockSeed, MONEYMAKER_VARIATION + 1);
    }
    ...
    if (!optimizePass) {
        for (unsigned int i = 0; i < 10; i++) {
            ...
            if (MoneyOptimizeEvaluateStocks(data, stocks, stockStart, result))
                break;
        }
    ...
    for (unsigned int i = 0; i < FIRESTARTER_OPTIMIZE_ITERATIONS; i++) {
        ...
        if (MoneyOptimizeEvaluateStocks(data, stocks, stockStart, curResult))
            result = curResult;
        else
            data[d] = oldData;
    }
...
inline bool MoneyTesterEvaluate(const FireStarterData& data, const MoneyMakerStock& stockData, MoneyMakerStock& resultData, unsigned int& trades, float& result)
{
    ...
    // Use the evaluation to trade the stock.
    for (unsigned int i = 0; i < MONEYMAKER_VARIATION + MONEYMAKER_TRADING; i++) {
        float newPrice = stockData[index];
        float priceChange = newPrice / oldPrice;
        oldPrice = newPrice;
