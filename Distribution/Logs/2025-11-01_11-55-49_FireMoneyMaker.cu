...
inline bool MoneyMakerEvaluate(const FireStarterData& data, const FireStarterCode& code, const MoneyMakerStock& stock, float& result)
{
...
inline bool MoneyMakerEvaluateStocks(const FireStarterData& data, const FireStarterCode& code, const MoneyMakerStocks& stocks, float& result)
{
    ...
    for (unsigned int stock = 0; stock < MONEYMAKER_STOCKS; stock++) {
        ...
        if (!MoneyMakerEvaluate(data, code, stocks.Stock(stock), stockResult)) {
...
GPU_GLOBAL void MoneyMaker(float* results, FireStarterResult* population, FireStarterCode* codes, MoneyMakerStocks* inputStocks, const unsigned long long evolveSeed, const unsigned int passes, const unsigned int populationSize)
{
