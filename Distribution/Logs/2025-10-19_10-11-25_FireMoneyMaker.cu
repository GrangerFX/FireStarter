...
inline bool MoneyMakerEvaluate(const FireStarterData& data, const FireStarterCode& code, const MoneyMakerStock& stockData, unsigned int& trades, float& result)
{
...
GPU_GLOBAL void MoneyMaker(float* results, FireStarterResult* population, FireStarterCode* codes, MoneyMakerStocks* stocks, const unsigned long long seed, const unsigned int passes, const unsigned int populationSize)
{
    ...
    // Get the stock data.
    const MoneyMakerStock& stockData = stocks->Stock(0);
    ...
    for (unsigned int i = 0; i < 10; i++) {
        ...
        if (MoneyMakerEvaluate(data, code, stockData, memberTrades, memberResult))
            break;
    }
    ...
    for (unsigned int pass = 0; pass < passes; pass++) {
        ...
        for (unsigned int i = 0; i < FIRESTARTER_EVOLVE_GPU_ITERATIONS; i++) {
            ...
            if (MoneyMakerEvaluate(data, code, stockData, curTrades, curResult)) {
