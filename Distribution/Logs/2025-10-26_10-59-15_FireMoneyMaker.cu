...
inline bool MoneyMakerEvaluate(const FireStarterData& data, const FireStarterCode& code, const MoneyMakerStock& stock, const unsigned int stockStart, float& result)
{
...
inline bool MoneyMakerEvaluateStocks(const FireStarterData& data, const FireStarterCode& code, const MoneyMakerStocks* stocks, const unsigned int* stockStart, float& result)
{
    float stocksResult = 0.0f;
    for (unsigned int stock = 0; stock < MONEYMAKER_STOCKS; stock++) {
        float stockResult = FIRESTARTER_START_RESULT;
        if (!MoneyMakerEvaluate(data, code, stocks->Stock(stock), stockStart[stock], stockResult)) {
...
GPU_GLOBAL void MoneyMaker(float* results, FireStarterResult* population, FireStarterCode* codes, MoneyMakerStocks* stocks, const unsigned long long evolveSeed, const unsigned int passes, const unsigned int populationSize)
{
    ...
    for (unsigned int i = 0; i < 10; i++) {
        ...
        if (MoneyMakerEvaluateStocks(data, code, stocks, stockStart, result))
            break;
    }
    ...
    for (unsigned int pass = 0; pass < passes; pass++) {
        ...
        for (unsigned int i = 0; i < FIRESTARTER_EVOLVE_GPU_ITERATIONS; i++) {
            ...
            if (MoneyMakerEvaluateStocks(data, code, stocks, stockStart, curResult))
                result = curResult;
            else
                data[d] = old;
        }
