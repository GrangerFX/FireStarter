...
// Run duration = 188.935624 seconds
// Run generation = 81
// Run evolution = 5
...
inline bool Evaluate(FireStarterData& testData, float &n)
{
    FireStarterData data(testData);
    // EVALUATE //
    n += data[0];
...
//inline float TestEvaluate(const FireStarterData& data, const float target[FIRESTARTER_SAMPLES], const float theta[FIRESTARTER_SAMPLES])
inline bool TestEvaluate(FireStarterData& data, const float target[], const float theta[], float &result)
{
    result = 0.0f;
    for (int i = 0; i < FIRESTARTER_SAMPLES; i++) {
        float n = theta[i];
...
GPU_GLOBAL void Optimizer(const FireStarterSettings settings, FireStarterPopulation* newResults, const FireStarterPopulation* oldResults, const unsigned int v, const unsigned int registers, const unsigned long long optimizationSeed, const unsigned long long optimizationPass)
{
    ...
    for (unsigned int p = 0; p < settings.m_iterations; p++) {
        ...
        data[d] = oldData + evolutionScale * RANDOMFACTOR(seed);
        float curResult;
        TestEvaluate(data, target, theta, curResult);
        if (curResult <= result)
            result = curResult;
        else
            data[d] = oldData;
