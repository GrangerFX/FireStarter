...
inline bool MoneyOptimizeEvaluate(const FireStarterData& data, const MoneyMakerStock& stock, float& result)
{
    float startingFunds = MONEYMAKER_FUNDS;
    float funds = startingFunds;
    unsigned int index = stock.startDay;
...
inline bool MoneyOptimizeEvaluateStocks(const FireStarterData& data, const MoneyMakerStocks& stocks, float& result)
{
    ...
    for (unsigned int stock = 0; stock < MONEYMAKER_STOCKS; stock++) {
        ...
        if (!MoneyOptimizeEvaluate(data, stocks.Stock(stock), stockResult)) {
...
GPU_GLOBAL void MoneyOptimizer(FireStarterResult* newPopulation, const FireStarterResult* oldPopulation, MoneyMakerStocks* inputStocks, const unsigned int registers, const unsigned long long optimizeSeed, const unsigned long long optimizePass, unsigned int populationSize)
{
    ...
    // Local copy of the stock data in shared memory.
    GPU_SHARED MoneyMakerStocks stocks;
    stocks.Copy(inputStocks);
