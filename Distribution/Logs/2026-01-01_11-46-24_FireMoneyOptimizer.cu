...
inline bool MoneyTesterEvaluate(const FireStarterSettings* settings, const FireStarterData& data, const MoneyMakerStock& stock, MoneyMakerStock& trades, float& result)
{
    ...
    // The result is the ratio between the starting funds and the final funds.
    // Note: This ratio is inverted to prefer smaller numbers for compatibility with FireStarter.
    trades[0] = startingFunds / tradingFunds; // Inverse alpha.
    return true;
} // MoneyTesterEvaluate
...
GPU_GLOBAL void MoneyTester(const FireStarterSettings* settings, const MoneyMakerStocks* stocks, MoneyMakerStocks* tradingResults, const FireStarterData* tradingData)
{
    ...
    if (stockIndex < settings->m_stocks) {
        ...
        MoneyTesterEvaluate(settings, *tradingData, stocks->Stock(stockIndex + settings->m_stock), tradingResults->Stock(stockIndex + settings->m_stock), testResult);
        int foo = 1;
    }
} // MoneyTester
