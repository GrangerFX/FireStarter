...
#else
inline bool MoneyOptimizeEvaluateStock(const FireStarterSettings* settings, const FireStarterData& data, const MoneyMakerStock& stock, unsigned long long seed, float& result)
{
    ...
    for (unsigned int session = 0; session < settings->m_sessions; session++) {
        ...
        if (!MoneyOptimizeEvaluate(settings, data, stock, sessionStart, sessionResult))
            return false;
        stockResult += sessionResult;
    }
    result = stockResult / settings->m_sessions;
    return true;
} // MoneOptimizeEvaluateStock
...
inline bool MoneyOptimizeEvaluateStocks(const FireStarterSettings* settings, const FireStarterData& data, const MoneyMakerStocks& stocks, unsigned long long seed, float& result)
{
    ...
    for (unsigned int stock = 0; stock < settings->m_stocks; stock++) {
        ...
        if (!MoneyOptimizeEvaluateStock(settings, data, stocks.Stock(stock), stockSeed, stockResult))
            return false;
        stocksResult += stockResult;
    }

    result = stocksResult / settings->m_stocks;
    return true;
} // MoneyOptimizeEvaluateStocks
#endif
...
GPU_GLOBAL void MoneyOptimizer(const FireStarterSettings* settings, FireStarterResult* newPopulation, const FireStarterResult* oldPopulation, MoneyMakerStocks* stocks, const unsigned int registers, const unsigned long long optimizeSeed, const unsigned long long optimizePass, unsigned int populationSize)
{
    ...
    if (!optimizePass) {
        for (unsigned int i = 0; i < 10; i++) {
            ...
            if (MoneyOptimizeEvaluateStocks(settings, data, *stocks, optimizeSeed, result) && (result < settings->m_startResult))
                break;
        }
        ...
        if (evolveAge > 1) {
            ...
            if (MoneyOptimizeEvaluateStocks(settings, data, *stocks, optimizeSeed, result) && (result < memberResult))
    ...
    for (unsigned int i = 0; i < FIRESTARTER_OPTIMIZE_ITERATIONS; i++) {
        ...
        if (MoneyOptimizeEvaluateStocks(settings, data, *stocks, optimizeSeed, curResult) && (curResult < result))
            result = curResult;
        else
            data[d] = oldData;
    }
