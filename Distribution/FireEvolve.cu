...
// EVALUATE //
inline float Evaluate(FireStarterData data, float n, unsigned int thread)
{
...
inline float TestEvaluate(const FireStarterData& data, const float target[], const float theta[], unsigned int thread)
{
    float result = 0.0f;
    for (int i = 0; i < FIRESTARTER_SAMPLES; i++)
        result = fmaxf(fabsf(Evaluate(data, theta[i], thread) - target[i]), result);
    return result;
} // TestEvaluate

GPU_GLOBAL void Evolver(const FireStarterSettings settings, FireStarterPopulation* newResults, const FireStarterPopulation* oldResults, const unsigned int v, const unsigned int registers, const unsigned long long optimizationSeed, const unsigned long long optimizationPass)
{
