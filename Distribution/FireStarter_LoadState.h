#pragma once
#include "FireStarterState.h"

inline void LoadSettings(FireStarterSettings& settings)
{
    settings.m_variations = 3;
    settings.m_samples = 15;
    settings.m_instructions = 32;
    settings.m_registers = 32;
    settings.m_opcodes = 2;

    settings.m_targetMin = 0.000000f;
    settings.m_targetMax = 6.283185f;

    settings.m_mode = FIRESTARTER_UNIT;
    settings.m_units = 4;
    settings.m_processes = 0;
    settings.m_population = 69632;
    settings.m_iterations = 128;
    settings.m_candidates = 16;
    settings.m_generations = 100;
    settings.m_precision = 256;
    settings.m_evolve = FIRESTARTER_EVOLVE_BEST;
    settings.m_attempts = 32;
    settings.m_seed = 3494;
    settings.m_scale = 0.100000f;
    settings.m_startScale = 4.000000f;
    settings.m_startResult = 10.000000f;
} // LoadSettings

inline void LoadProgram(FireStarterProgram& program)
{
    FireStarterSettings settings;
    LoadSettings(settings);
    program.InitProgram(settings);
    program.m_dataSize = 17;
    program.m_maxRegisters = 9;

    program.m_registers.resize(17);
    program.m_registers[0] = {0, 0, 0, 24};
    program.m_registers[1] = {1, 1, 1, 11};
    program.m_registers[2] = {2, 2, 2, 20};
    program.m_registers[3] = {3, 3, 3, 25};
    program.m_registers[4] = {4, 4, 5, 10};
    program.m_registers[5] = {5, 5, 6, 26};
    program.m_registers[6] = {6, 6, 8, 8};
    program.m_registers[7] = {7, 6, 9, 21};
    program.m_registers[8] = {8, 1, 12, 27};
    program.m_registers[9] = {9, 4, 13, 30};
    program.m_registers[10] = {10, 10, 14, 14};
    program.m_registers[11] = {11, 7, 15, 31};
    program.m_registers[12] = {12, 8, 16, 28};
    program.m_registers[13] = {13, 13, 18, 18};
    program.m_registers[14] = {14, 14, 19, 19};
    program.m_registers[15] = {15, 15, 23, 23};
    program.m_registers[16] = {16, 16, 29, 29};

    FireStarterInstructions* instructions = program.Instructions();
    instructions->SetOperation(0, 1, 0);
    instructions->SetOperation(1, 0, 1);
    instructions->SetOperation(2, 0, 2);
    instructions->SetOperation(3, 1, 3);
    instructions->SetOperation(4, 1, 3);
    instructions->SetOperation(5, 1, 4);
    instructions->SetOperation(6, 1, 5);
    instructions->SetOperation(7, 0, 4);
    instructions->SetOperation(8, 0, 6);
    instructions->SetOperation(9, 0, 7);
    instructions->SetOperation(10, 0, 4);
    instructions->SetOperation(11, 1, 1);
    instructions->SetOperation(12, 1, 8);
    instructions->SetOperation(13, 1, 9);
    instructions->SetOperation(14, 0, 10);
    instructions->SetOperation(15, 0, 11);
    instructions->SetOperation(16, 1, 12);
    instructions->SetOperation(17, 1, 3);
    instructions->SetOperation(18, 0, 13);
    instructions->SetOperation(19, 1, 14);
    instructions->SetOperation(20, 0, 2);
    instructions->SetOperation(21, 1, 7);
    instructions->SetOperation(22, 0, 12);
    instructions->SetOperation(23, 1, 15);
    instructions->SetOperation(24, 0, 0);
    instructions->SetOperation(25, 1, 3);
    instructions->SetOperation(26, 0, 5);
    instructions->SetOperation(27, 0, 8);
    instructions->SetOperation(28, 0, 12);
    instructions->SetOperation(29, 1, 16);
    instructions->SetOperation(30, 0, 9);
    instructions->SetOperation(31, 1, 11);
} // LoadProgram

inline void LoadVariation0(FireStarterResult* result)
{
    FireStarterData *data = result->Data(0);
    data->d[0] = -0.192368f;
    data->d[1] = 0.435436f;
    data->d[2] = -0.002177f;
    data->d[3] = -0.275789f;
    data->d[4] = 0.291815f;
    data->d[5] = -2.246365f;
    data->d[6] = -0.536703f;
    data->d[7] = -0.526785f;
    data->d[8] = -0.305662f;
    data->d[9] = -1.104004f;
    data->d[10] = 1.038170f;
    data->d[11] = -0.960583f;
    data->d[12] = 1.101444f;
    data->d[13] = -9.215259f;
    data->d[14] = -6.100288f;
    data->d[15] = 0.464559f;
    data->d[16] = 0.473249f;
    data->d[17] = 0.000000f;
    data->d[18] = 0.000000f;
    data->d[19] = 0.000000f;
    data->d[20] = 0.000000f;
    data->d[21] = 0.000000f;
    data->d[22] = 0.000000f;
    data->d[23] = 0.000000f;
    data->d[24] = 0.000000f;
    data->d[25] = 0.000000f;
    data->d[26] = 0.000000f;
    data->d[27] = 0.000000f;
    data->d[28] = 0.000000f;
    data->d[29] = 0.000000f;
    data->d[30] = 0.000000f;
    data->d[31] = 0.000000f;
    *result->MinResult(0) = 0.000344f;
} // LoadVariation0

inline void LoadVariation1(FireStarterResult* result)
{
    FireStarterData *data = result->Data(1);
    data->d[0] = -0.657685f;
    data->d[1] = -0.892304f;
    data->d[2] = -0.001065f;
    data->d[3] = 0.028917f;
    data->d[4] = 0.924155f;
    data->d[5] = -2.162051f;
    data->d[6] = 1.285666f;
    data->d[7] = -0.094648f;
    data->d[8] = 0.541267f;
    data->d[9] = 1.697419f;
    data->d[10] = -0.307255f;
    data->d[11] = -1.287282f;
    data->d[12] = 0.793316f;
    data->d[13] = 6.710983f;
    data->d[14] = 0.829754f;
    data->d[15] = 0.360653f;
    data->d[16] = 0.344124f;
    data->d[17] = 0.000000f;
    data->d[18] = 0.000000f;
    data->d[19] = 0.000000f;
    data->d[20] = 0.000000f;
    data->d[21] = 0.000000f;
    data->d[22] = 0.000000f;
    data->d[23] = 0.000000f;
    data->d[24] = 0.000000f;
    data->d[25] = 0.000000f;
    data->d[26] = 0.000000f;
    data->d[27] = 0.000000f;
    data->d[28] = 0.000000f;
    data->d[29] = 0.000000f;
    data->d[30] = 0.000000f;
    data->d[31] = 0.000000f;
    *result->MinResult(1) = 0.000183f;
} // LoadVariation1

inline void LoadVariation2(FireStarterResult* result)
{
    FireStarterData *data = result->Data(2);
    data->d[0] = -0.264375f;
    data->d[1] = -0.350077f;
    data->d[2] = -0.469018f;
    data->d[3] = -1.122223f;
    data->d[4] = 1.183963f;
    data->d[5] = -1.619948f;
    data->d[6] = 0.004359f;
    data->d[7] = -2.160211f;
    data->d[8] = -0.770270f;
    data->d[9] = -0.634983f;
    data->d[10] = -0.318017f;
    data->d[11] = -0.601197f;
    data->d[12] = 0.593979f;
    data->d[13] = -1.137448f;
    data->d[14] = 1.513618f;
    data->d[15] = 0.396428f;
    data->d[16] = -0.474138f;
    data->d[17] = 0.000000f;
    data->d[18] = 0.000000f;
    data->d[19] = 0.000000f;
    data->d[20] = 0.000000f;
    data->d[21] = 0.000000f;
    data->d[22] = 0.000000f;
    data->d[23] = 0.000000f;
    data->d[24] = 0.000000f;
    data->d[25] = 0.000000f;
    data->d[26] = 0.000000f;
    data->d[27] = 0.000000f;
    data->d[28] = 0.000000f;
    data->d[29] = 0.000000f;
    data->d[30] = 0.000000f;
    data->d[31] = 0.000000f;
    *result->MinResult(2) = 0.000343f;
} // LoadVariation2

inline void LoadResult(FireStarterState& state)
{
    LoadVariation0(state.Result());
    LoadVariation1(state.Result());
    LoadVariation2(state.Result());

    state.Result()->maxResult = 0.000344f;
} // LoadResult

inline void LoadState(FireStarterState& state)
{
    state.m_generation = 18;
    state.m_seed = 3028357566;

    LoadProgram(state.m_program);
    LoadResult(state);
} // LoadState
